...
class DumpHooks {
	...
	public function __invoke( $args ) {
		...
		foreach ( $runtime->get_calls() as $class_name => $hooks ) {
			...
			if ( 'BracketSpace\\Notification\\Signature\\Runtime' === $class_name ) {
				$callback_object_name = '$this';
			} else {
				$component_name = array_search( $class_name, $objects, true );
				if ( ! $component_name ) {
					WP_CLI::warning( str_replace( 'BracketSpace\\Notification\\Signature\\', '', $class_name ) . ' skipped, no instance available' );
					continue;
				}
			...
			WP_CLI::log( str_replace( 'BracketSpace\\Notification\\Signature\\', '', $class_name ) . ' added ' . $count . ' hooks' );
		}
